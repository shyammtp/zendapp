...
var App = function () {
	...
	$(window).scroll(function(e) {
	    ...
	    var scroller_anchor = $(".scroller_anchor").offset().top + 110;
	     
	    // Check if the user has scrolled and the current position is after the scroller start location and if its not already fixed at the top 
	    if ($(this).scrollTop() >= scroller_anchor && $('.fixedPopup').length == 0) 
	    {    // Change the CSS of the scroller to hilight it and fix it at the top of the screen. 
	       //$('body').prepend(fixedbtn);
	       var current = fixedbtn.clone().prependTo('body');
	       current.addClass('fixedPopup');
		$(".fixedPopup").css({
		    'position': 'fixed',
		    'top': '40px',
		    'width' :'100%',
		...
		$(".fixedPopup .form-actions").prepend("<div class='fixed-title'>"+pageTitle+"</div>");
		$('.fixedPopup .form-actions').css({'background':'rgba(245, 245, 245,1)','border-bottom':'1px solid #ccc','padding': '11px 20px 11px'});
		
		 $('.fixedPopup').animate({'opacity' : 1},500);
	    } 
	    else if ($(this).scrollTop() < scroller_anchor && $('.fixedPopup').length > 0) 
